...
if (isset($_SESSION['foodie_user'])) {
    ...
    {
	    //Print shopping list
	    $sql_shopping = "SELECT id, recipe, ingredients FROM shopping";
        ...
        else if ($_POST['action'] == "sl_delete")
        {
        ...
        else if ($_POST['action'] == "sl_print")
        {
	        //Printer friendly version of shopping list
	        //Print shopping list
	        $sql_shopping = "SELECT recipe, ingredients FROM shopping";
	        if (!$exec_shopping = $dbconnect->query($sql_shopping))
	        {
		        echo "<p class=\"error\">" . ERROR_SHOPPING_RETRIEVE_LIST . "<br>\n" . $exec_shopping->error();
	        }
