...
//This function adds HTML header file
function cs_AddHeader() 
{	global $trans_sid;
	include(dirname(__FILE__)."/includes/header.inc.php");
}
//This function adds HTML footer file
...
function cs_CountRowsRecipePage()
{
	...
	$num_rows_per_page = mysqli_num_rows($exec_db_browse);
...
function cs_PrintRecipeData()
{
	...
	while ($recipe_data = mysqli_fetch_array($exec_recipe))
	{
		$gray = 1;
		$colors[0] = "#dddddd";
		$colors[1] = "#eeeeee";
...
function cs_PrintModifyTable()
{
	...
	$arr_element = 0;
	$line_number = 1;
	while ($recipe_browse_list = mysqli_fetch_object($exec_db_browse)) 
	{
		$list_data[$arr_element][0] = $recipe_browse_list->id;
		$list_data[$arr_element][1] = $recipe_browse_list->name;
		$list_data[$arr_element][2] = $line_number;
